...
def register_view(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            OTPHandler.send_otp_email(user, otp, purpose='registration')
...
def login_view(request):
    """User login view - sends OTP to email"""
    if request.user.is_authenticated:
        return redirect('dashboard:home')
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            try:
                ...
                # Generate OTP
                otp = user.generate_otp()
                OTPHandler.send_otp_email(user, otp, purpose='login')
...
def verify_otp_view(request):
    ...
    # Make sure we have the email variable for the template
    context = {
        'form': form,
        'email': user.email,
...
def resend_otp_view(request):
    ...
    try:
        otp = user.generate_otp()
        OTPHandler.send_otp_email(user, otp, purpose=purpose)
